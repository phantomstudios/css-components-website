...
```tsx
import type * as CSSComponents from "@phantomstudios/cssdios/css-components";
```

These include:

- `CSSComponents.CSS` - The case type of css values, a string or array of css selectors.
- `CSSComponents.CSSComponentConfig` - The type for the config object passed to `styled`.
...
```tsx
import { styled, VariantProps } from "@phantomstudios/css-components";
