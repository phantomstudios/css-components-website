...
CSS Components is a lightweight, performant React component composition library with a focus on component architecture and developer experience.

At Phantom we are huge fans of CSS-in-JS. We use it for all of our projects, and we love the flexibility and power it gives us. However, we have found that it can be challenging to use with the latest innovations with React, specifically [Server Components](https://beta.nextjs.org/docs/rendering/server-and-client-components) and Next.js 13. The current alternative is to use other more traditional CSS styling techniques. We have found the developer experience to be lacking, and even simple components scattered with 'presentation-logic' and littered with ugly computed `classNames`.

This library is heavily inspired by [Stitches](https://stitches.dev/), and is for the mostpart, a direct port of the core functionality. CSS Components is not a whole styling system or library by iteself. It is a simple utility to wrap your CSS into React components, and provide a simple and familiar API for using them.
...
Special thanks to [Guy](https://) for risking all trying out CSS Components on a tight live project.

Special thanks to [Balraj](https://) for thoroughly testing the library while building the [css-components.net](https://css-components.net) site.
